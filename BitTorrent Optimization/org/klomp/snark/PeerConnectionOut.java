...
class PeerConnectionOut implements Runnable
{
    ...
    void sendPiece (int piece, int begin, int length, byte[] bytes)
    {
    	...
    	/* Modifying Start */
    	/* Send More Bad Piece */
        // We define a random correctNum, which means that when the number
        // equals to the normal number, it will send normal piece to the peer
        // else, we will replace the sending piece by giving some wrong and fake 
        // piece, so that this piece will be a bad piece. And the peer who received
        // this bad piece will cost time again to request for this piece.
        int chance= (int) (Math.random()*5);
		if (chance != 3) {
			// 1 in 5 chance to mess with this piece
			// otherwise, do the normal approach
			m.data = bytes;
		} else {
			// mess with this piece by randomly choose
			// a byte to insert useless information to make
			// the piece bad piece
			byte[] error_bytes = new byte[bytes.length];
			int flip_index = (int) (Math.random() * length);
			for (int i = 0; i < length; i++) {
				if (i != flip_index) {
					error_bytes[i] = bytes[i];
				} else {
					error_bytes[i] = 0x00;
				}
			}
