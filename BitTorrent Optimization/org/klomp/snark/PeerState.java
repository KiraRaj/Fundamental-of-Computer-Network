...
class PeerState
{
    ...
    void haveMessage (int piece)
    {
        ...
        if (piece < 0 || piece >= metainfo.getPieces()) {
            ...
            /* Modifying Start */
            /* When the condition fails, means the message sent to us is wrong
             * some where, so we treat it as a bad action*/
        	peer.bad_request();
        	//if the peer give a bad request, record it and try to make it blacklisted
        ...
        throws IOException
    {
        ...
        if (piece < 0 || piece >= metainfo.getPieces() || begin < 0
            ...
            /* Modifying Start */
            /* When ths sanity check is fails, means that the peer sent us 
             * an insanity piece, we treat it as a kind of bad action */
            peer.bad_request();
        ...
        if (pieceBytes == null) {
            ...
            /* Modifying Start */
            /* When the message we received is null, means that the peer sent us
             * an empty piece, we also treat this as a bad action */
            peer.bad_request();
        ...
        if (begin >= pieceBytes.length || begin + length > pieceBytes.length) {
            ...
            /* Modifying Start */
            /* When the piece we received is not equal to the piece we request,
             * means that the peer send us an unwanted piece, although it not
             * a really bad action, we still treat it as bad*/
            peer.bad_request();
        ...
        /* Modifying Start */
        /* Add maximum try times */
        // Here, if the peer reach the set maximum try times, we will
        // send it a requested piece, and at the same time reset the try count.
        // Otherwise, we will just add 1 to the try count but not send a request
        // piece.
		if (peer.reach_max_retry()) {
			...
			peer.increase_max_retry();
		} else {
			peer.increase_retry_counter();
		}
    }
        ...
        throws IOException
    {
        ...
        if (getFirstOutstandingRequest(req.piece) == -1) {
            if (listener.gotPiece(peer, req.piece, req.bs)) {
                ...
                /* Modifying Start */
                /* When the condition fails, means we received a bad piece
                 * some where, so we treat it as a bad action*/
                peer.give_bad_piece();
    ...
    Request getOutstandingRequest (int piece, int begin, int length)
    {
        ...
        if (r == -1) {
            ...
            /* Modifying Start */
            /* When the number of request is not the same as what we requested,
             * means that we received some unrequested piece, we will again treat
             * it as a bad action */
            peer.give_bad_piece();
