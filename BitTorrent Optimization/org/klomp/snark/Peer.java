...
public class Peer implements Comparable<Peer>
{
    ...
    // Adding a bad_action_count(initial 0) to account for the bad action
    // a peer has done to us, when the bad_action_count is larger than the
    // limitation of bad action, we will push the peer into blacklist
    private int bad_action_count = 0;
    private int bad_action_limit = 5;
    
    private int MAX_RETRY=1;
    
    private int retry_counter=0;
    
    //if the peer give a bad piece, record it
    public void give_bad_piece(){
    	bad_action_count++;
    }
    
    //if the peer give a bad request, record it
    public void bad_request(){
    	bad_action_count++;
    }
    
    // When the bad_action_count is larger than the base value of limitation
    // the peer will call this inBlackList() function to show that itself is 
    // in the blacklist   
    public boolean check_blacklist(){
    	if(bad_action_count>=bad_action_limit){
    		System.err.println(this.toString()+" is blacklisted!");
    		return true;
    	}
    ...
    // Every round, if a peer does not perform some bad action, and at the same
    // time, its bad_action_count is not 0, we will deduct 1 to show that this
    // peer perform good this round. Also, when the account is deducted to a value
    // less than the limitation, the peer will leave blacklist
    public void whitelist(){
    	if (bad_action_count>=1){
    		bad_action_count--;
    	}  	
    }
    
    // Attack strategy to set up a counter and wait a little
    // longer every time
    
    // Everytime calling this function, we will add 1 to the try_count
    public void increase_max_retry(){
    	MAX_RETRY++;
    	retry_counter=0;
    }
    
    public void increase_retry_counter(){
    	retry_counter++;
    }
    
    public boolean reach_max_retry(){
    	return (retry_counter>=MAX_RETRY);
    }
    ...
    /* Modifying Start */
    /* Simulating BitTyrant */
    
    // We can access to the upload and download rate, and using Dp/Up to
    // reorder the peers, so that the peers list will not choose peer and 
    // add peers randomly, but will based on the ratio to reorder them, so that
    // peer who send much more frequently will be placed at the very beginning
    public double dp_ratio(){
    	if(this.getUploaded()!=0){
    		return this.getDownloaded()/this.getUploaded();
    	}else{
    		return 0;
    	}
    }
